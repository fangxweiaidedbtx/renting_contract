...
contract RentalContract {
    ...
    function rentProperty(uint256 _propertyId, bytes32 _hashLock, uint256 _timeLock) public payable {
        ...
        property.nextRentDue = block.timestamp + 30 days; // 假设每月续租
    ...
    function unlockHTLC(uint256 _propertyId, bytes32 _preimage) public {
        ...
        property.tenant = address(0); // 重置租客信息
    ...
    function refundHTLC(uint256 _propertyId) public {
        ...
        // 将资金退还给房客
        payable(property.tenant).transfer(address(this).balance);
        property.isRented = false; // 重置租赁状态
        property.hashLock = 0x0; // 重置哈希锁
        property.timeLock = 0; // 重置时间锁
        property.nextRentDue = 0; // 重置下一次租金到期时间
    ...
    function renewLease(uint256 _propertyId) public payable {
        ...
        require(block.timestamp >= property.nextRentDue, "Current lease period has not ended");

        // 更新下一次租金到期时间
        property.nextRentDue = block.timestamp + 30 days; // 假设租期为每月一次
        // 将租金支付给房东
