...
contract RentalContract {
    ...
    // 发布房源信息的功能函数
    function publishProperty(string memory _addr, uint256 _area, uint256 _rent,string memory _contactIinformation) public payable{
    ...
    function refundHTLC(uint256 _propertyId) public {
        ...
        require(address(this).balance >= property.rent + property.deposit  + recovery_fee/2 , "Contract balance is insufficient");
    ...
    function renewLease(uint256 _propertyId) public payable {
        ...
        require(msg.value == property.rent, "your given value is not equal to the rent");
    ...
    function getAllProperties() public view returns (RentalProperty[] memory) {
        RentalProperty[] memory allProperties = new RentalProperty[](propertyCount);
    ...
    function returnDeposit(uint256 _propertyId) public {
        RentalProperty storage property = properties[_propertyId];
        require(property.isRented, "Property is not rented");
        require(msg.sender == property.tenant, "Only tenant can return deposit");
